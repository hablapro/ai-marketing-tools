/*
  # Create tools and related tables
  ...
  1. New Tables
    ...
    - `tool_fields`: Stores form fields for each tool
      - `id` (uuid, primary key)
    ...
    - `hero_sections`: Stores hero section content for each tool
      - `id` (uuid, primary key)
      - `tool_id` (uuid, foreign key)
      - `title` (text)
      - `description` (text)
...
BEGIN
  ...
  DROP POLICY IF EXISTS "Allow full access for admins" ON tool_fields;
  DROP POLICY IF EXISTS "Allow full access for admins" ON hero_sections;
...
BEGIN
  ...
  DROP TRIGGER IF EXISTS update_tool_fields_updated_at ON tool_fields;
